...
-(id)initWithSelectTitle:(NSString *)title viewOfDelegate:(UIView *)view delegate:(id<CDPDatePickerDelegate>)delegate{
    if (self=[super init]) {
        ...
        _datePickerView.backgroundColor=[MyController colorWithHexString:@"F7F7F7"];
        ...
        _dateLabel=[[UIButton alloc] initWithFrame:CGRectMake(0,0,_view.bounds.size.width/2,_view.bounds.size.height*0.07042)];
//        if (title) {
...
//        _dateLabel.backgroundColor=[UIColor clearColor];
        [_dateLabel setTitle:@"取消" forState:UIControlStateNormal];
        _dateLabel.userInteractionEnabled=YES;
        [_dateLabel addTarget:self action:@selector(dateCancelClick) forControlEvents:UIControlEventTouchUpInside];
        ...
        [_dateConfirmButton setTitleColor:[MyController colorWithHexString:@"58CDB3"] forState:UIControlStateNormal];

...
- (void)viewDidLoad {
    ...
    if (self.conversation.type == EMConversationTypeChat) {
        ...
        [clearButton setImage:[UIImage imageNamed:@"删除聊天"] forState:UIControlStateNormal];
    ...
    for (EMMessage *message in messages) {
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:modelForMessage:)]) {
            ...
            model.failImageName = @"imageDownloadFail";
                model.nickname = dic[@"selfNickName"];
                model.avatarURLPath = dic[@"selfHeadImageStr"];
        }
...
- (void)sendTextMessage:(NSString *)text
{
    ...
    if (self.conversation.type == EMConversationTypeChat) {
        NSDictionary *externMeaage = @{@"selfNickName":[MyController returnStr:self.selfNickName],@"selfHeadImageStr":[MyController returnStr:self.selfHeadImageStr],@"friendNickName":[MyController returnStr:self.friendNickName],@"friendHeadImageStr":[MyController returnStr:self.friendHeadImageStr],@"selfUserId":[MyController returnStr:self.selfUserId],@"friendUserId":[MyController returnStr:self.friendUserId],@"sex":[MyController returnStr:self.sex],@"fsex":[MyController returnStr:self.fsex]};
//        NSDictionary *externMeaage = @{@"userId":[MyController returnStr:self.userId],@"acatar":[MyController returnStr:self.acatar],@"nickname":[MyController returnStr:self.nickname]};
            ext = externMeaage;
    }
    [self sendTextMessage:text withExt:ext];
}

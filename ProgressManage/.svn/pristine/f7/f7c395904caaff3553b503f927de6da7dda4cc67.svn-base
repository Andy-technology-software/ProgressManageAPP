...
+ (void)initialize
{
    ...
    cell.detailLabelColor = [MyController colorWithHexString:@"81889d"];
    cell.detailLabelFont = [UIFont systemFontOfSize:12];
    cell.timeLabelColor = [MyController colorWithHexString:@"c8cbd4"];
    cell.timeLabelFont = [UIFont systemFontOfSize:12];
    cell.line.backgroundColor = [MyController colorWithHexString:@"EEEFF2"];
}
...
- (void)_setupSubview
{
    ...
    _line.backgroundColor = [MyController colorWithHexString:@"EEEFF2"];
...
//    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.line attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeBottom multiplier:1.0 constant:-5]];
//    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.line attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeLeft multiplier:1.0 constant:EaseConversationCellPadding ]];
//    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.line attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeRight multiplier:1.0 constant:-EaseConversationCellPadding]];
//    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.line attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeHeight multiplier:0 constant:5]];
    self.line.frame = CGRectMake(10, 69, SCREEN_WIDTH - 40, 1);
}
/*!
 @method
...
{
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.avatarView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeTop multiplier:1.0 constant:EaseConversationCellPadding * 1.5]];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.avatarView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeBottom multiplier:1.0 constant:-EaseConversationCellPadding * 1.5]];
        ...
        if([subView isKindOfClass:NSClassFromString(@"UITableViewCellDeleteConfirmationView")])
        {
            for (UIButton *btn in subView.subviews) {
                if ([btn isKindOfClass:[UIButton class]]) {
                    //更改左滑标签按钮样式
                    if ([btn.titleLabel.text isEqualToString:@"聊天置顶"]) {
                        [btn setTitle:@"    聊天置顶" forState:UIControlStateNormal];
                        btn.titleLabel.font = [UIFont systemFontOfSize:13];
                        [btn setTitleColor:[MyController colorWithHexString:@"838793"] forState:UIControlStateNormal];
                        [btn setBackgroundImage:[UIImage imageNamed:@"置顶2"] forState:UIControlStateNormal];
                        btn.imageView.contentMode = UIViewContentModeScaleAspectFit;
                        btn.backgroundColor = [UIColor whiteColor];
                    }else if([btn.titleLabel.text isEqualToString:@"取消置顶"]){
                        [btn setTitle:@"    取消置顶" forState:UIControlStateNormal];
                        btn.titleLabel.font = [UIFont systemFontOfSize:13];
                        [btn setTitleColor:[MyController colorWithHexString:@"838793"] forState:UIControlStateNormal];
                        [btn setBackgroundImage:[UIImage imageNamed:@"置顶2"] forState:UIControlStateNormal];
                        btn.imageView.contentMode = UIViewContentModeScaleAspectFit;
                        ...
                        btn.titleLabel.font = [UIFont systemFontOfSize:13];
                        [btn setBackgroundImage:[UIImage imageNamed:@"凌信-删除2"] forState:UIControlStateNormal];
                        btn.imageView.contentMode = UIViewContentModeScaleAspectFit;
                        btn.backgroundColor = [UIColor whiteColor];

...
@interface GemScrollView()<UIScrollViewDelegate>
...
@property (nonatomic,strong)ChangeFramePageControl *pageControl;

@end

@implementation GemScrollView
...
- (void)loadBaseView
{
    ...
    for (int i = 0; i<3; i ++) {
        ...
        GemScrollViewItem *itemView = [[GemScrollViewItem alloc] initWithFrame:CGRectMake(i*self.scrollView.frame.size.width, 0, self.scrollView.frame.size.width, self.scrollView.frame.size.height - 30)];
        NSLog(@"------%@",NSStringFromCGRect(itemView.frame));
        
        if (i == 0 && self.dataArry!=nil && self.dataArry.count > 1) {
            itemView.model = self.dataArry[2];

        }
        if (i == 1 && self.dataArry!=nil && self.dataArry.count > 0) {
            itemView.model = self.dataArry[0];

        }
        if (i == 2 && self.dataArry !=nil && self.dataArry.count > 1) {
            itemView.model = self.dataArry[1];
...
- (UIColor *)color_pageControl
{
    if (!_color_pageControl) {
        _color_pageControl = [MyController colorWithHexString:@"dfe1e7"] ;
    }
...
- (UIColor *)color_currentPageControl
{
    if (!_color_currentPageControl) {
        _color_currentPageControl = [MyController colorWithHexString:@"4d8af4"] ;
    }
...
- (void)reloadImage
{
    ...
    GemScrollViewItem * centerImageView = [self.imageViews objectAtIndex:1];
    GemScrollViewItem *leftImageView = [self.imageViews objectAtIndex:0];
    GemScrollViewItem *rightImageView = [self.imageViews objectAtIndex:2];
    
    centerImageView.model =self.dataArry[_currentImageIndex];
    ...
    leftImageView.model  = self.dataArry[leftImageIndex] ;
    rightImageView.model = self.dataArry[rightImageIndex] ;
...
- (ChangeFramePageControl *)pageControl
{
    if (!_pageControl) {
        _pageControl = [[ChangeFramePageControl alloc] init] ;
